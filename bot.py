...
FOOTBALL_KEYWORDS = [
    "فوتبال", "استقلال", "پرسپولیس", "سپاهان", "تراکتور",
    "ذوب آهن", "گل گهر", "نساجی", "تیم ملی", "لیگ برتر",
    "جام حذفی", "جام جهانی", "لیگ قهرمانان", "لیگ اروپا",
    "لژیونر", "طارمی", "آزمون", "جهانبخش", "قایدی",
    "رونالدو", "مسی", "رئال مادرید", "بارسلونا",
    "منچستریونایتد", "منچسترسیتی", "لیورپول", "چلسی",
    "آرسنال", "بایرن مونیخ", "اینتر", "میلان",
    "یوونتوس", "پاری سن ژرمن"
]

HASHTAG_KEYWORDS = [
    "استقلال", "پرسپولیس", "سپاهان", "تراکتور", "ذوب آهن",
    "گل گهر", "نساجی", "تیم ملی", "لیگ برتر", "جام حذفی",
    "جام جهانی", "لیگ قهرمانان", "لیگ اروپا", "لژیونر",
    "طارمی", "آزمون", "جهانبخش", "قایدی", "رئال مادرید",
    "بارسلونا", "منچستریونایتد", "منچسترسیتی", "لیورپول",
    "چلسی", "آرسنال", "بایرن مونیخ", "اینتر", "میلان",
    "یوونتوس", "پاری سن ژرمن", "فوتبال ایران", "فوتبال اروپا"
]
...
def extract_aparat_hash(url):
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            if 5 <= len(video_hash) <= 10:
                return video_hash

    return None


def get_aparat_video_url(url):
    video_hash = extract_aparat_hash(url)
    ...
    print("APARAT HASH:", video_hash)
    print("APARAT API:", api_url)
    ...
    try:
        ...
        # اول کیفیت بالاتر را امتحان می‌کنیم
        quality_order = {
            "1080p": 1080,
            "720p": 720,
            "480p": 480,
        ...
        candidates.sort(
            key=lambda item: quality_order.get(
                item[0],
                0
            ),
            reverse=True
        ...
        for profile, mp4_url in candidates:
            try:
                test = requests.get(
                    mp4_url,
                    headers=HEADERS,
                ...
                content_type = test.headers.get(
                    "content-type",
                    ""
                ).lower()

                content_length = test.headers.get(
                    "content-length",
                    ""
                )
...
def get_video_url(article_url):
    ...
    try:
        ...
        # اگر خود لینک آپارات باشد
        if "aparat.com" in response.url.lower():
        ...
        # جستجوی لینک‌های آپارات داخل صفحه
        aparat_links = []

        for tag in soup.find_all(
            ["a", "iframe", "video", "source"]
        ):
            for attribute in [
                "href",
                "src",
        ...
        for aparat_link in aparat_links:
            print(
                "APARAT LINK FOUND:",
                aparat_link
            )
